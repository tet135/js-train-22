...
class Participant {
  constructor(alias, communicator) {
    this.alias = alias; //псевдомим(кому надіслати повідомлення)
    this.communicator = communicator; // спосіб комунікації
  }
  dispatchMessage(text) {
    const formatedText = this.prepareMessage(text);
    this.communicator.transmit(formatedText);
  }
